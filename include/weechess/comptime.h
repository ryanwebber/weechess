...
namespace {
    ...
    constexpr std::array<BitBoard, 64> compute_rook_slide_masks()
    {
        ...
        for (auto i = 0; i < 64; i++) {
            ...
            masks[i] |= rays[i][Direction::West] & ~file_mask[A_File];
            masks[i] |= rays[i][Direction::East] & ~file_mask[H_File];
            masks[i] |= rays[i][Direction::North] & ~rank_mask[_8th_Rank];
            masks[i] |= rays[i][Direction::South] & ~rank_mask[_1st_Rank];
        }
    ...
    constexpr std::array<BitBoard, 64> compute_bishop_slide_masks()
    {
        ...
        for (auto i = 0; i < 64; i++) {
            ...
            masks[i] |= rays[i][Direction::NorthWest] & ~(file_mask[A_File] | rank_mask[_8th_Rank]);
            masks[i] |= rays[i][Direction::SouthWest] & ~(file_mask[A_File] | rank_mask[_1st_Rank]);
            masks[i] |= rays[i][Direction::NorthEast] & ~(file_mask[H_File] | rank_mask[_8th_Rank]);
            masks[i] |= rays[i][Direction::SouthEast] & ~(file_mask[H_File] | rank_mask[_1st_Rank]);
        }
